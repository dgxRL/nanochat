...
def main():
    ...
    parser.add_argument('--model-tag', type=str, default="d6", help='nanochat model tag to identify the checkpoint directory')
    parser.add_argument('--step', type=int, default=5000, help='Model step to load (default = last)')
    parser.add_argument('--max-per-task', type=int, default=16, help='Max examples per CORE task (-1 = all)')
    parser.add_argument('--device-batch-size', type=int, default=1, help='Per-device batch size for BPB evaluation')
    parser.add_argument('--split-tokens', type=int, default=16384, help='Number of tokens to evaluate per split for BPB')
